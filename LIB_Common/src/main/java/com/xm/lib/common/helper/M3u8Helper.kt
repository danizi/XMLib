...
object M3u8Helper {
    ...
    fun parseDownUrl(m3u8: String) {
        OkHttpClient().newCall(Request.Builder().url(m3u8).build()).enqueue(object : Callback {
            ...
            override fun onResponse(call: Call, response: Response) {
                ...
                for (i in 0..73) {
                    val cache = "" + Environment.getExternalStorageDirectory() + File.separator + i + ".ts"
                    cachePaths.add(cache)
                }
                val cacheM3u8File = File("" + Environment.getExternalStorageDirectory() + File.separator + "26de49f8c253b3715148ea0ebbb2ad95_1.m3u8")
    ...
    fun getDownUrls(inputStream: InputStream?): ArrayList<String> {
