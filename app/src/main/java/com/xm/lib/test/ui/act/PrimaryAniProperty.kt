 ...
 */
class PrimaryAniProperty : AppCompatActivity() {
    ...
    private fun objectAnimatorTest(view: View) {
        if (isAnimByCode) {
            ...
            val set = AnimatorInflater.loadAnimator(this, R.animator.animator_object_animator) as AnimatorSet
            set.setTarget(view)
            set.start()
        }
    }
    ...
    private fun valueAnimatorTest(view: View) {
        if (isAnimByCode) {
            ...
            val valueAnimator = AnimatorInflater.loadAnimator(this, R.animator.animator_value_animator) as ValueAnimator
            valueAnimator.addUpdateListener {
                val curValue = it.animatedValue as Float
                BKLog.d("curValue:$curValue")
                //根据变化的值向右边滑动
    ...
    private fun animatorSetTest(view: View) {
        if (isAnimByCode) {
             ...
             * 执行的顺序是 animX -》animTranslationX $ animTranslationY -》animAlpha
             */
            ...
            set.setTarget(view)
            set.duration = 5000
            val anim = ObjectAnimator.ofFloat(view, "scaleX", 1.5f)
            val parentSet = AnimatorSet()
            val builder = parentSet.play(anim)
            builder.after(set)
            parentSet.start()
        } else {
            val set = AnimatorInflater.loadAnimator(this, R.animator.animator_animator_set) as AnimatorSet
            set.setTarget(view)
            set.start()
        }
    }
